...
if [ $CLEANUP = 'Y' ]; then
  ...
  echo "Generate Data!"
  /usr/bin/hive -u "jdbc:hive2://zk0-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk1-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk5-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2-interactive" -n "" -p "" -f TPCDSDataGen.hql -hivevar SCALE=3 -hivevar PARTS=10 -hivevar LOCATION=/HiveTPCDS/ -hivevar TPCHBIN=$(grep -A 1 "fs.defaultFS" /etc/hadoop/conf/core-site.xml | grep -o "abfs[^<]*")/tmp/resources --hivevar QUERY=TPCDSDataGen_$(date '+%Y%m%d_%H%M%S')
  echo "Create External Tables!"
  /usr/bin/hive -u "jdbc:hive2://zk0-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk1-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk5-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2-interactive" -n "" -p "" -f ddl/createAllExternalTables.hql -hivevar LOCATION=/HiveTPCDS/ -hivevar DBNAME=tpcds --hivevar QUERY=createAllExternalTables_$(date '+%Y%m%d_%H%M%S')
  echo "Analyze External Tables!"
  /usr/bin/hive -u "jdbc:hive2://zk0-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk1-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk5-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2-interactive" -n "" -p "" -f ddl/analyze.hql -hivevar ORCDBNAME=tpcds --hivevar QUERY=analyze_external_$(date '+%Y%m%d_%H%M%S')
fi

if [[ "$FORMAT" == "ALL" || "$FORMAT" == "Parquet" ]]; then
  if [[ "$GENERATE_TABLES" == "Y" ]]; then
    echo "Create Parquet Tables!"
    /usr/bin/hive -u "jdbc:hive2://zk0-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk1-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk5-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2-interactive" -n "" -p "" -i settings.hql -f ddl/createAllParquetTables.hql -hivevar PARQUETDBNAME=tpcds_parquet -hivevar SOURCE=tpcds --hivevar QUERY=createAllParquetTables_$(date '+%Y%m%d_%H%M%S')
  ...
  if [[ "$EXECUTE_QUERY" == "Y" ]]; then
    ...
    for f in queries/*.$SCALE.sql; do for i in {1..1}; do
     ...
     ((count++))
      ...
      /usr/bin/hive -u "jdbc:hive2://zk0-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk1-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk5-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2-interactive" -i settings.hql -f $f -hivevar ORCDBNAME=tpcds_parquet --hivevar QUERY=Parquet_$f.$(date '+%Y%m%d_%H%M%S') >$f.run_$i.out 2>&1
      SUCCESS=$?
      ENDTIME="$(date +%s)"
  ...
  if [[ "$GENERATE_TABLES" == "Y" ]]; then
    ...
    /usr/bin/hive -u "jdbc:hive2://zk0-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk1-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181,zk5-mas114.ce32grx1nkwevemuf3wjhlo2gh.cx.internal.cloudapp.net:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2-interactive" -n "" -p "" -i settings.hql -f ddl/createAllORCTables.hql -hivevar ORCDBNAME=tpcds_orc -hivevar SOURCE=tpcds --hivevar QUERY=createAllORCTables_$(date '+%Y%m%d_%H%M%S')
